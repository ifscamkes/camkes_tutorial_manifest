...
int /*? me.interface.name ?*/__run(void) {
    ...
    while (1) {
	//(RWFM) Generating Read call for RWFM label checking
	/*- if me.instance.name!='rwfm_monitor' and me.interface.name not in rwfm_from_end_interfaces.values() -*/
	  //ZF_LOGF_IF(/*? rwfm_from_end_interfaces[me.instance.name] ?*/_can_i_read(rwfm_sub_id, /*? me.interface.name ?*/_rwfm_obj_id) == false, "Read rule violation");
