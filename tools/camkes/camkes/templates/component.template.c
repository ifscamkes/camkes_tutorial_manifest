...
//(RWFM) CODE BOUND STARTS
/*- if me.name == 'rwfm_monitor' -*/

RWFM rwfm_monitor = {/*? rwfm.num_subjects ?*/,
					   /*? rwfm.num_objects ?*/,
					   {  /*- for i in rwfm.subjects.values(): -*/
					      { /*? str(i.sub_id_index) ?*/ , /*? str(i.owner) ?*/ , /*? str(i.readers) ?*/ , /*? str(i.writers) ?*/ }
						/*- if i != (rwfm.subjects.values())[-1]: -*/
						   , 
						/*- endif -*/ 
						/*- endfor -*/ },
					  {  /*- for i in rwfm.objects.values(): -*/
                                              { /*? str(i.obj_id_index) ?*/ , /*? str(i.owner) ?*/ , /*? str(i.readers) ?*/ , /*? str(i.writers) ?*/ }
                                                /*- if i != (rwfm.objects.values())[-1]: -*/ 
                                                   , 
                                                /*- endif -*/ 
                                                /*- endfor -*/ }};
/*- for parent, val in rwfm_from_end_interfaces.items(): -*/
  int /*?val?*/_parent_rwfm_sub_id = /*- for sub in rwfm_subjects.values(): -*/
				       /*- if parent == sub[0]: -*/
					/*? sub[1] ?*/;
				       /*- endif -*/
				     /*- endfor -*/
/*- endfor-*/
/*- else -*/
int rwfm_sub_id = /*- for sub in rwfm_subjects.values(): -*/
		    /*- if me.instance.name == sub[0]: -*/
		      /*? sub[1] ?*/;
		    /*- endif -*/
		  /*- endfor -*/
/*- endif -*/
//(RWFM) CODE BOUND ENDS
