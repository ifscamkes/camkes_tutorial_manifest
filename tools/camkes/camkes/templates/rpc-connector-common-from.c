...
/*- if me.instance.name!='rwfm_monitor' and me.interface.name not in rwfm_from_end_interfaces.values() -*/
int /*? me.interface.name ?*/_rwfm_obj_id = /*- for sub in rwfm_objects.values(): -*/
					      /*- if me.interface.name == sub[0]: -*/
						/*? sub[1] ?*/;
					      /*- endif -*/
					    /*- endfor -*/
/*- endif-*/
...
/*- endif -*/
    ...
    //(RWFM) BOUND STARTS HERE
    //Generating Write and Read call check for RWFM
    /*- if me.instance.name!='rwfm_monitor' and me.interface.name not in rwfm_from_end_interfaces.values() -*/
        //ZF_LOGF_IF(/*? rwfm_from_end_interfaces[me.instance.name] ?*/_can_i_write(rwfm_sub_id, /*? me.interface.name ?*/_rwfm_obj_id) == false, "Write rule violation");
        if (!/*? rwfm_from_end_interfaces[me.instance.name] ?*/_can_i_write(/*? me.interface.name ?*/_rwfm_obj_id)) return ;
        if (!/*? rwfm_from_end_interfaces[me.instance.name] ?*/_can_i_read(/*? me.interface.name ?*/_rwfm_obj_id)) return ;
    /*- endif -*/
    //#(RWFM) BOUND ENDS HERE
    ...
    //#(RWFM) BOUND STARTS HERE
    /*- if me.instance.name!='rwfm_monitor' and me.interface.name not in rwfm_from_end_interfaces.values() -*/
      /*? rwfm_from_end_interfaces[me.instance.name] ?*/_update_my_label(/*? me.interface.name ?*/_rwfm_obj_id);
    /*- endif -*/
    //#(RWFM) BOUND ENDS HERE
